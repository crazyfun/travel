...
?>
      <?php $cssPath=$this->get_css_path();?>
        <div class="page-right-top">点击日期进行程设置</div>
	  ...
	  $form=$this->beginWidget('EActiveForm', array(
         ...
         ?>
            		<table cellspacing="0" cellpadding="0">
                        <tbody><tr>
                            <td width="100" height="30" align="right"><strong>旅行社名称:</strong></td>
                            <td>
                            	<?php echo EHtml::createText("agency_name",$page_params['agency_name'],array());?>
                             </td>
  ...
  $this->widget('zii.widgets.grid.CGridView',array(
    ...
    'columns'=>array(
            array(
                ...
                'value'=>'$data->show_attribute("ordain_id")',//如果为空则以$data->id为值 如果有值则 已$data->func()填充
            ),
            array(
                'name'=>'线路名称',
                'type'=>'raw',//字段的属性 参考yii
...
function get_scheduler_datas(year,month){
	jQuery.getJSON("/api/guidecalendar", { year: year, month: month }, function(json){
  			if(json.flag=='y'){
  				var guide=json.datas.guide;
					...
					for(var i = 1, len = monthDay; i <= len; i++){
						...
						if(guide_key){
						  ...
						  if(status=='2'){
						  	innerhtml+="<div class='date_item'><a class='text_gray' href=\"javascript:view_guide('"+current_guide.id+"')\">" + i + "</a></div";
						  }else{
						  	innerhtml+="<div class='date_item'><a href=\"javascript:add_guide('"+current_date+"')\">" + i + "</a></div>";
						  } 
						}else{
							innerhtml+="<div class='date_item'><a href=\"javascript:add_guide('"+current_date+"')\">" + i + "</a></div>";
						}
...
function view_guide(guide_id){
	// 用iframe显示http://www.baidu.com的内容，并设置了标题、宽与高、按钮
jQuery.jBox("iframe:/user/viewguidedate?id="+guide_id, {
    title: "查看行程",
    width: 800,
    height: 300,
    buttons: { '关闭': true }
...
function add_guide(current_date){
	// 用iframe显示http://www.baidu.com的内容，并设置了标题、宽与高、按钮
jQuery.jBox("iframe:/user/addguidedate?current_date="+current_date, {
    title: "增加行程",
    width: 800,
    height: 300,
    buttons: { '关闭': true }
...
function view_ordain(ordain_id){
	// 用iframe显示http://www.baidu.com的内容，并设置了标题、宽与高、按钮
jQuery.jBox("iframe:/user/viewordain?id="+ordain_id, {
    title: "查看预定信息",
    width: 800,
    height: 600,
    buttons: { '关闭': true }
