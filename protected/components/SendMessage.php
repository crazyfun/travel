...
class SendMessage{
  ...
  //发送会员升级邮件
 public function send_message($id="",$user_id,$cellphone,$content_array){
 	if(empty($cellphone))
		 return;
	...
	$content=$this->replace_variable($content_array);
  	...
  	$all_syscfg_values=$sys_config->get_all_syscfg();
		...
		$conpon=$user_data->conpon;
		if($conpon>=$sfc_message_consume){
			
			$result=$this->message_class->send($cellphone,mb_convert_encoding($content['content'],"gb2312","UTF-8"));
			if($result){
				$consume_temp=new ConsumeTemp();
				$consume_temp->consume('15',$user_id,'2',$sfc_message_consume,array('value'=>$sfc_message_consume));
			}
		}
	return $result;
}
