       ...
       if(!empty($create_id)){
        ...
        for($ii=0;$ii<$diff_month;$ii++){
        	...
        	array_push($conditions,"DATE_FORMAT(t.date,'%Y-%m')=:current_date");
    		  ...
    		  $total_sell=$model->get_total_profit($conditions,$params);
    		  $dataSet->addPoint(new Point($current_date."(".$total_sell.")", $total_sell));
        }
				$chart->setDataSet($dataSet);
				$chart->setTitle("月份利润趋势图");
	      $image_path="upload/generated/".$domain."/sanke";
	      Util::makeDirectory($image_path);
	      $chart->render($image_path."/profit.png");
    	  $this->display('profit',array('model'=>$model,'image_path'=>$image_path));
    } 
}
?>
