...
/**
 * This is the model class for table "{{ordain}}".
 *
 * The followings are the available columns in table '{{ordain}}':
 * @property string $id
 ...
 */
class Ordain extends BaseActive
{
	/**
	 * Returns the static model of the specified AR class.
	 * @return Ordain the static model class
	 */
	...
	public function rules()
	{
		...
		return array(
			...
			array('user_id, ordain_id, date_id,', 'length', 'max'=>11),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, user_id, ordain_id, date_id,status,create_time, operate_time', 'safe', 'on'=>'search'),
		);
	}
 ...
 public function insert_datas(){
	...
	function beforeSave(){
	  if(parent::beforeSave()){
		...
		switch($attribute_name){
		  case 'user_id':
		    $guide=Guide::model();
		    $guide_data=$guide->find('user_id=:user_id',array(':user_id'=>$this->User->id));
		    return CHtml::link($this->User->real_name,Yii::app()->getController()->createUrl("/guide/guide/detail",array('id'=>$guide_data->id)),array('target'=>'_self'));
		    break;
		  case 'ordain_id':
		    return CHtml::link($this->OrderUser->TravelAgency->agency_name,Yii::app()->getController()->createUrl("/agency/agency/detail",array('id'=>$this->OrderUser->TravelAgency->id)),array('target'=>'_self'));
		    break;
		  case 'date_id':
	...
	function get_calendar($start_date,$end_date){
		...
		$datas=$this->with("User","OrderUser","AgencyDate")->findAll(array(
		  ...
		  'condition'=>'AgencyDate.start_date>=:start_date AND AgencyDate.end_date<=:end_date AND t.user_id=:user_id',
		...
		foreach($datas as $key => $value){
			...
			$tem_array['line']=$value->OrderUser->TravelAgency()->agency_name."<span class='text_079270'>(".$value->AgencyDate->line.")</span>";
	...
	function get_agency_calendar($start_date,$end_date){
		$user_id=Yii::app()->user->id;
		$datas=$this->with("User","OrderUser","AgencyDate")->findAll(array(
		  'select'=>'id,user_id,ordain_id,date_id,status',
		...
		foreach($datas as $key => $value){
			...
			$tem_array['user_id']=$value->user_id;
			$tem_array['ordain_id']=$value->ordain_id;
			$tem_array['line']=$value->User->real_name."<span class='text_079270'>(".$value->AgencyDate->line.")</span>";
			$tem_array['start_date']=$value->AgencyDate->start_date;
			$tem_array['end_date']=$value->AgencyDate->end_date;
			$tem_array['status']=$value->status;
			$ajax_array[]=$tem_array;
