  ...
  /*
	...
	*/
	public function show_friendlink_img(){
		return CHtml::image("/".$this->friendlink_img,"",array("width"=>'50','height'=>'50'));
	}
	
 function friend_link_datas(){
		$friendlink_type=FriendlinkType::model();
		$friendlink_type_datas=$friendlink_type->findAll(array('select'=>'id,type_name','condition'=>'','params'=>array(),'order'=>'create_time DESC'));
		$friendlink_datas=array();
		foreach($friendlink_type_datas as $key => $value){
			$tem_friendlink=array();
			$tem_friendlink['category']=$value->type_name;
			$fiendlink_data=$this->findAll(array('select'=>'friendlink_name,friendlink_img,friendlink_href','condition'=>'friendlink_type=:friendlink_type','params'=>array(':friendlink_type'=>$value->id),'order'=>'friendlink_sort ASC'));
			$tem_friendlink['friendlink']=$fiendlink_data;
			array_push($friendlink_datas,$tem_friendlink);
		}
		return $friendlink_datas;
		
	}
	
	function get_friend_link(){
			$fiendlink_data=$this->findAll(array('select'=>'friendlink_name,friendlink_img,friendlink_href','condition'=>'','params'=>array(),'order'=>'friendlink_sort ASC'));
			return $fiendlink_data;
	}
}