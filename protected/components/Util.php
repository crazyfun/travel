...
public static function db_create_in($item_list, $field_name = '')
{
  ...
  public static function search_in_region($region_id=""){
  	$region=Region::model();
  	 if(empty($region_id)){
		...
		$value=Yii::app()->cache->get("InRegion".$region_id);
		if($value===false)
		{
			//$parents=$region->get_parents($region_id);
		  $descendants=$region->get_descendant($region_id);
		  $value=array();
		  //$value=array_unique(array_merge($parents,$descendants));
		  $value=array_unique($descendants);
      Yii::app()->cache->set("InRegion".$region_id,$value);
		}
		return Util::db_create_in($value); 
  }
  
  
  
  public static function com_ad_position(){
  	$ad_position=CV::$ad_position;
  	$return_array=array();
  	foreach($ad_position as $key => $value){
  		$return_array[$key]=$value['name']."(宽:".$value['width'].",高:".$value['height'].")";
  	}
  	return $return_array;
  } 
  public static function GetAd($type_id,$region_id='',$expire="60*60*24"){
  	...
  	$value=Yii::app()->cache->get("Advertising_".$type_id."_".$region_id);
		if($value===false)
		{
			$advertising=Advertising::model();
			$advertising_data=$advertising->find("type_id=:type_id AND FIND_IN_SET('".$region_id."',region_id)>0",array(':type_id'=>$type_id));
			$ad_position=CV::$ad_position[$type_id];
			$value='<div class="image_ad" style="width:'.$ad_position['width'].'px;height:'.$ad_position['height'].';">'.$advertising_data->content.'</div>';
      Yii::app()->cache->set("Advertising_".$type_id."_".$region_id,$value,$expire);
		}
		return $value; 
  }
...
public static function get_cache_id($id){
	    $ip_convert=IpConvert::get();
		  $region_session=$ip_convert->init_region();  	 		 	    
			$region_id=$region_session['id'];
	    return $id."_".$region_id;
}
