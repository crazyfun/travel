...
?>
 <?php $cssPath=$this->get_css_path();?>
        <div class="page-right-top">点击日期添加计调线路</div>
        ...
        <div class="page-bottom-tip" style="padding-left:500px;"><div class="show_yellow_bg">无预定</div><div class="show_red_bg">有预定</div></div>
     </div>
...
function get_scheduler_datas(year,month){
	jQuery.getJSON("/api/myagencycalendar", { year: year, month: month }, function(json){
  			if(json.flag=='y'){
...
jQuery.jBox("iframe:/user/addagencydate?current_date="+current_date, {
    title: "增加线路信息",
    width: 800,
    height: 450,
    buttons: { '关闭': true }
...
function view_ordain(ordain_id){
	// 用iframe显示http://www.baidu.com的内容，并设置了标题、宽与高、按钮
jQuery.jBox("iframe:/user/viewagencyordain?id="+ordain_id, {
    title: "查看预定信息",
    width: 800,
    height: 600,
    buttons: { '关闭': true }
