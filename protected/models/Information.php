...
class Information extends BaseActive
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('content,short_content', 'safe'),
			array('sort','length','max'=>'4'),
			array('information_image','length','max'=>'100'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, model_type, type_id, title, content,short_content,views, create_id, create_time', 'safe', 'on'=>'search'),
		);
	}
  ...
  function show_attribute($attribute_id){
	...
	function get_newest_information($model_type,$nums=10){
		$information_datas=$this->with("User",'Type')->findAll(array('select'=>'t.id,t.type_id,t.title,t.create_time','condition'=>'t.model_type=:model_type','params'=>array(':model_type'=>$model_type),'limit'=>$nums,'order'=>'t.create_time DESC'));
	...
	function get_newest_ginformation($limit=10,$type_id=''){
		$conditions=array();
		$params=array();
		if(!empty($type_id)){
			$condition[]="type_id=:type_id";
		...
		$model_type=empty($model_type)?CV::$model_type['guideinfo']:$model_type;
		$condition[]="model_type=:model_type";
	  $params[':model_type']=$model_type;
	  $information_datas=$this->findAll(array('condition'=>implode(" AND ",$condition),'params'=>$params,'order'=>'create_time DESC','limit'=>$limit));
	  return $information_datas;
	}
	
	function get_hotest_ginformation($limit=10,$type_id=''){
		$conditions=array();
		$params=array();
		if(!empty($type_id)){
			$condition[]="type_id=:type_id";
		...
		$condition[]="model_type=:model_type";
	  $params[':model_type']=$model_type;
	  $information_datas=$this->findAll(array('condition'=>implode(" AND ",$condition),'params'=>$params,'order'=>'views DESC','limit'=>$limit));
	  return $information_datas;
	}
