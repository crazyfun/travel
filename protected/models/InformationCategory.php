...
class InformationCategory extends BaseActive
{
	...
	function get_parent_select($model_type){
	   	$information_category_datas=$this->findAll(array('select'=>'id,parent_id,name','condition'=>'model_type=:model_type','params'=>array(':model_type'=>$model_type),'order'=>'parent_id ASC'));
	   	$parent_select=array();
	   	foreach($information_category_datas as $key => $value){
	   	  $parent_select[$value->id]=$value->name;	
	   	}
